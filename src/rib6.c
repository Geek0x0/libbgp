...
typedef struct rib6_lpm_key {
    libbgp_prefix6_t prefix;
} rib6_lpm_key_t;

typedef struct rib6_lpm_entry {
    libbgp_prefix6_t prefix;
    bgp_hashmap_entry_t **entries;
    size_t count;
    size_t cap;
...
typedef struct rib6_neighbor_as_cache {
    uint32_t value;
    bool ready;
    bool valid;
} rib6_neighbor_as_cache_t;
...
static libbgp_prefix6_t rib6_lpm_normalize_prefix(const libbgp_prefix6_t *prefix)
{
    libbgp_prefix6_t normalized;
    uint8_t mask[16];
    size_t i;

    memset(&normalized, 0, sizeof(normalized));
    if (prefix == NULL || prefix->len > 128u) {
        return normalized;
    }
    normalized = *prefix;
    libbgp_cidr6_to_mask(prefix->len, mask);
    for (i = 0u; i < sizeof(normalized.addr); i++) {
        normalized.addr[i] &= mask[i];
    }
    return normalized;
}

static uint64_t rib6_lpm_hash(const void *key, void *ctx)
{
    const rib6_lpm_key_t *k = (const rib6_lpm_key_t *)key;
    uint64_t h = k->prefix.len;
    size_t i;
...
static bool rib6_lpm_key_eq(const void *a, const void *b, void *ctx)
{
    const rib6_lpm_key_t *ka = (const rib6_lpm_key_t *)a;
    const rib6_lpm_key_t *kb = (const rib6_lpm_key_t *)b;
...
static void rib6_lpm_entry_free(void *key, void *value, void *ctx)
{
    rib6_lpm_entry_t *entry = (rib6_lpm_entry_t *)value;
...
static bool rib6_neighbor_as_cached(
    const libbgp_rib6_route_t *route,
    rib6_neighbor_as_cache_t *cache)
{
    if (cache == NULL) {
        return false;
    }
    if (!cache->ready) {
        cache->value = 0u;
        cache->valid = rib6_neighbor_as(route, &cache->value);
        cache->ready = true;
    }
    return cache->valid;
}

static bool rib6_better_cached(
    const libbgp_rib6_route_t *a,
    rib6_neighbor_as_cache_t *a_cache,
    const libbgp_rib6_route_t *b,
    rib6_neighbor_as_cache_t *b_cache)
{
    uint32_t a_lp;
    uint32_t b_lp;
    ...
    if (a->med != b->med && rib6_neighbor_as_cached(a, a_cache) &&
        rib6_neighbor_as_cached(b, b_cache) && a_cache->value == b_cache->value) {
        return a->med < b->med;
    }
...
static libbgp_err_t rib6_lpm_index_add_locked(
    rib6_impl_t *impl,
    bgp_hashmap_entry_t *route_entry);

static void rib6_lpm_index_remove_locked(
    rib6_impl_t *impl,
    bgp_hashmap_entry_t *route_entry);
...
static size_t rib6_remove_source_locked(rib6_impl_t *impl, uint32_t source_router_id)
{
    ...
    for (i = 0u; i < source->count; i++) {
        ...
        if (rib6_route_entry_unlink_locked(impl, entry)) {
            rib6_lpm_index_remove_locked(impl, entry);
    ...
    const libbgp_prefix6_t *prefix)
{
    ...
    rib6_lpm_index_remove_locked(impl, match_entry);
...
static libbgp_err_t rib6_attach_detached_locked(rib6_impl_t *impl, bgp_hashmap_entry_t *entry)
{
    ...
    if (rib6_source_index_add(impl, entry) != LIBBGP_OK) {
        (void)rib6_route_entry_unlink_locked(impl, entry);
        return LIBBGP_ERR_NOMEM;
    }
    if (rib6_lpm_index_add_locked(impl, entry) != LIBBGP_OK) {
        rib6_source_index_remove(impl, entry);
        (void)rib6_route_entry_unlink_locked(impl, entry);
        return LIBBGP_ERR_NOMEM;
    }
    ...
    const libbgp_prefix6_t *prefix)
{
    ...
    rib6_neighbor_as_cache_t best_cache = { 0u, false, false };
    ...
    for (entry = impl->routes.buckets[idx]; entry != NULL; entry = entry->next) {
        libbgp_rib6_route_t *route = (libbgp_rib6_route_t *)entry->value;
        rib6_neighbor_as_cache_t route_cache = { 0u, false, false };

        if (entry->hash == hash && rib6_key_eq(entry->key, &find_key, NULL) &&
            libbgp_prefix6_eq(&route->prefix, prefix) &&
            (best == NULL || rib6_better_cached(route, &route_cache, best, &best_cache))) {
            best = route;
...
static rib6_lpm_entry_t *rib6_lpm_group_find_locked(
    rib6_impl_t *impl,
    const libbgp_prefix6_t *prefix)
{
    rib6_lpm_key_t find_key;

    if (impl == NULL || prefix == NULL || prefix->len > 128u) {
        return NULL;
    }
    find_key.prefix = rib6_lpm_normalize_prefix(prefix);
    return (rib6_lpm_entry_t *)bgp_hashmap_find_first(&impl->lpm_groups, &find_key);
}

static libbgp_rib6_route_t *rib6_lpm_group_best_locked(const rib6_lpm_entry_t *group)
{
    libbgp_rib6_route_t *best = NULL;
    rib6_neighbor_as_cache_t best_cache = { 0u, false, false };
    size_t i;
    ...
    for (i = 0u; i < group->count; i++) {
        libbgp_rib6_route_t *route = (libbgp_rib6_route_t *)group->entries[i]->value;
        rib6_neighbor_as_cache_t route_cache = { 0u, false, false };

        if (best == NULL || rib6_better_cached(route, &route_cache, best, &best_cache)) {
            best = route;
            best_cache = route_cache;
        }
    }
...
static void rib6_lpm_group_remove_locked(rib6_impl_t *impl, rib6_lpm_entry_t *group)
{
    rib6_lpm_key_t find_key;
...
static void rib6_lpm_index_remove_locked(rib6_impl_t *impl, bgp_hashmap_entry_t *route_entry)
{
    ...
    for (i = 0u; i < group->count; i++) {
        if (group->entries[i] == route_entry) {
            ...
            if (group->count == 0u) {
                (void)bgp_radix6_remove(&impl->lpm_tree, group->prefix.addr, group->prefix.len);
                rib6_lpm_group_remove_locked(impl, group);
                return;
            }
            if (removed_best || group->best == NULL) {
                group->best = rib6_lpm_group_best_locked(group);
                if (group->best != NULL) {
                    (void)bgp_radix6_insert(
                        &impl->lpm_tree,
                        group->prefix.addr,
                        group->prefix.len,
                        group->best);
...
static libbgp_err_t rib6_lpm_index_add_locked(rib6_impl_t *impl, bgp_hashmap_entry_t *route_entry)
{
    libbgp_rib6_route_t *route;
    rib6_lpm_key_t find_key;
    rib6_lpm_entry_t *group;
    ...
    route = (libbgp_rib6_route_t *)route_entry->value;
    find_key.prefix = rib6_lpm_normalize_prefix(&route->prefix);
    group = (rib6_lpm_entry_t *)bgp_hashmap_find_first(&impl->lpm_groups, &find_key);
    if (group == NULL) {
        rib6_lpm_key_t *key;

        key = (rib6_lpm_key_t *)bgp_malloc(sizeof(*key));
        if (key == NULL) {
            return LIBBGP_ERR_NOMEM;
        }
        group = (rib6_lpm_entry_t *)bgp_calloc(1u, sizeof(*group));
        if (group == NULL) {
            bgp_free(key);
            return LIBBGP_ERR_NOMEM;
        }
    ...
    group->count++;
    if (group->best == NULL) {
        libbgp_err_t err = bgp_radix6_insert(
            &impl->lpm_tree,
            group->prefix.addr,
            group->prefix.len,
            route);
        ...
        if (rib6_better_cached(route, &route_cache, group->best, &best_cache)) {
            libbgp_err_t err = bgp_radix6_insert(
                &impl->lpm_tree,
                group->prefix.addr,
                group->prefix.len,
    ...
    const libbgp_prefix6_t *prefix)
{
    ...
    if (*count >= *cap) {
        ...
        next = (libbgp_prefix6_t *)bgp_realloc(*items, new_cap * sizeof(**items));
        if (next == NULL) {
            return LIBBGP_ERR_NOMEM;
        }
        *items = next;
    ...
    if (err == LIBBGP_OK) {
        ...
        if (err != LIBBGP_OK) {
            ...
            if (err != LIBBGP_OK) {
                ...
                err = rib6_lpm_index_add_locked(impl, new_entry);
                if (err != LIBBGP_OK) {
                    rib6_source_index_remove(impl, new_entry);
                    if (rib6_route_entry_unlink_locked(impl, new_entry)) {
                        rib6_entry_free(new_entry->key, new_entry->value, NULL);
                        bgp_free(new_entry);
                    }
                }
    ...
    for (i = 0u; i < source->count; i++) {
        ...
        if (best == NULL || route->prefix.len > best->prefix.len ||
            (route->prefix.len == best->prefix.len &&
                rib6_better_cached(route, &route_cache, best, &best_cache))) {
            best = route;
            best_cache = route_cache;
        }
    }
...
{
    rib6_impl_t *impl = rib6_impl_get(rib);
    libbgp_rib6_route_t *snapshots = NULL;
    ...
    for (i = 0u; i < impl->lpm_groups.bucket_count && err == LIBBGP_OK; i++) {
        ...
        for (entry = impl->lpm_groups.buckets[i]; entry != NULL; entry = entry->next) {
            rib6_lpm_entry_t *group = (rib6_lpm_entry_t *)entry->value;
            ...
            if (snap_count >= snap_cap) {
                ...
                next = (libbgp_rib6_route_t *)bgp_realloc(snapshots, next_cap * sizeof(*snapshots));
                if (next == NULL) {
                    err = LIBBGP_ERR_NOMEM;
                    break;
                }
            ...
            err = bgp_rib6_route_snapshot_clone(group->best, &snapshots[snap_count]);
            if (err != LIBBGP_OK) {
                break;
            }
            snap_count++;
    ...
    for (i = 0u; i < snap_count; i++) {
        rib6_route_snapshot_clear(&snapshots[i]);
    }
    bgp_free(snapshots);
    return err;
}
    ...
    bgp_rib6_discard_result_t *result)
{
    ...
    if (source != NULL) {
        for (i = 0u; i < source->count && err == LIBBGP_OK; i++) {
            ...
            if (route->source_router_id == source_router_id &&
                ...
                err = rib6_prefix_array_push(&active_prefixes, &active_count, &active_cap, &route->prefix);
                if (err != LIBBGP_OK) {
                    break;
                }
            }
    ...
    for (i = 0u; i < active_count && err == LIBBGP_OK; i++) {
        ...
        if (replacement == NULL) {
            err = rib6_prefix_array_push(
                &result->withdrawn,
                &result->withdrawn_count,
                &withdrawn_cap,
                &active_prefixes[i]);
        } else {
            err = rib6_result_add_replacement(result, &replacement_cap, replacement);
        }
    }
    bgp_unlock(&impl->lock);
    bgp_free(active_prefixes);
