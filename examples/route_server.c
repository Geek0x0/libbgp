...
static const char *event_name(libbgp_event_type_t type)
{
    switch (type) {
    case LIBBGP_EVENT_ROUTE_ADDED:
        return "route added";
    case LIBBGP_EVENT_ROUTE_WITHDRAWN:
        return "route withdrawn";
    case LIBBGP_EVENT_SESSION_UP:
        return "session up";
    case LIBBGP_EVENT_SESSION_DOWN:
        return "session down";
    case LIBBGP_EVENT_CUSTOM:
    default:
        return "custom";
    }
}

static void event_printer(const libbgp_event_t *event, void *ctx)
{
    libbgp_rib4_t *rib = (libbgp_rib4_t *)ctx;
    ...
    if (rib != NULL) {
        printf(" rib4=%" PRIu64, (uint64_t)libbgp_rib4_route_count(rib));
    }
    printf("\n");
}
...
int main(int argc, char **argv)
{
    ...
    rc = libbgp_rib4_init(&rib);
    if (rc != LIBBGP_OK) {
        fprintf(stderr, "rib4 init: %s\n", libbgp_strerror((libbgp_err_t)rc));
        return 1;
    }
    rc = libbgp_event_bus_init(&bus);
    if (rc != LIBBGP_OK) {
        fprintf(stderr, "event bus init: %s\n", libbgp_strerror((libbgp_err_t)rc));
    ...
    printf("route server listening on 0.0.0.0:%u\n", (unsigned int)port);
    ...
    close(listen_fd);
    rc = run_session(client_fd, &config, &rib, &bus);
    close(client_fd);
    libbgp_event_bus_destroy(&bus);
    libbgp_rib4_destroy(&rib);
    return rc;
